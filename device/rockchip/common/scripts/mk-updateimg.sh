...
build_updateimg()
{
	...
	if [ ! -f "$RK_ROCKDEV_DIR/filename.txt" ]; then
		FILENAME="update.img"
	else
		while IFS= read -r line; do
			FILENAME="$line"
		done < "$RK_ROCKDEV_DIR/filename.txt"
	fi

	TARGET="${1:-$RK_ROCKDEV_DIR/$FILENAME}"
	TYPE="${2:-update}"
	...
	"$RK_PACK_TOOL_DIR/rkImageMaker" -$TAG MiniLoaderAll.bin \
		update.raw.img $FILENAME -os_type:androidos

	ln -rsf "$IMAGE_DIR/package-file" "$OUT_DIR"
	ln -rsf "$IMAGE_DIR/$FILENAME" "$OUT_DIR"
	ln -rsf "$IMAGE_DIR/$FILENAME" "$TARGET"
