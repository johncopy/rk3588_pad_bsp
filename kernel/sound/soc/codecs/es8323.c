...
static const DECLARE_TLV_DB_SCALE(bypass_tlv, 0, 600, 0);
...
static void spk_delay_work(struct work_struct *work)
{
#ifdef CONFIG_SND_SOC_AW87XXX
	int ret;
	ret = aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);
	if(ret < 0)
		pr_err("%s: aw87xxx_pa1 set profile[%s] failed\n",
				__func__, aw_profile[1]);

	ret = aw87xxx_set_profile(AW_DEV_1, aw_profile[0]);
	if(ret < 0)
		pr_err("%s: aw87xxx_pa2 set profile[%s] failed\n",
				__func__, aw_profile[1]);
#endif
...
static int es8323_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		schedule_delayed_work(&es8323->spk_delayed_work, msecs_to_jiffies(50));
	}
...
static int es8323_probe(struct snd_soc_component *component)
{
	...
	snd_soc_component_write(component, 0x12, 0xd2);
	snd_soc_component_write(component, 0x13, 0xa2);
	...
	snd_soc_component_write(component, 0x1A, 0x01);
	snd_soc_component_write(component, 0x1B, 0x01);
			    ...
			    const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&es8323->spk_delayed_work, spk_delay_work);
