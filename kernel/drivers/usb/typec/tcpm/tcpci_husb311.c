...
#define HUSB311_TCPC_CMD_RESETTXBUFFER 0XDD
			    ...
			    bool on, bool charge)
{
    ...
    if (chip->charge_on == charge) {
		dev_dbg(chip->dev, "charge is already %s", charge ? "On" : "Off");
	} else {
		chip->charge_on = charge;

		if (!charge) {
			dev_dbg(chip->dev, "husb311 reset txbuffer");

			ret = husb311_write8(chip, TCPC_COMMAND, HUSB311_TCPC_CMD_RESETTXBUFFER);
			if (ret < 0)
				goto done;
		}
	}
