...
struct tcpm_port {
	...
	struct dentry *dentry_re_nego_5v;
...
static int re_nego_5v_show(struct seq_file *s, void *v)
{
	struct tcpm_port *port = (struct tcpm_port *)s->private;
        // 限制sink端只提供第一个PDO
        port->nr_snk_pdo = 1;
        tcpm_pd_hard_reset(port);
	return 0;
}
DEFINE_SHOW_ATTRIBUTE(re_nego_5v);
...
static void tcpm_debugfs_init(struct tcpm_port *port)
{
	...
	snprintf(name, NAME_MAX, "tcpm-%s-re-nego-5v", dev_name(port->dev));
	port->dentry_re_nego_5v = debugfs_create_file(name, S_IFREG | 0444, usb_debug_root,
					   port, &re_nego_5v_fops);
}
...
static void tcpm_debugfs_exit(struct tcpm_port *port)
{
	...
	debugfs_remove(port->dentry_re_nego_5v);
}
