...
static int hx_panel_coords[4] = {0, 1600, 0, 2560};
static int hx_display_coords[4] = {0, 2560, 0, 1600};
		...
		struct himax_i2c_platform_data *pdata)
{
	...
	/* imax_tpd_rst_gpio_number = GTP_RST_PORT; */
	/* imax_tpd_int_gpio_number = GTP_INT_PORT; */
...
int himax_ts_register_interrupt(void)
{
	...
	if (himax_tpd_int_gpio_number > 0) {
		himax_touch_irq = gpio_to_irq(himax_tpd_int_gpio_number);
...
module_i2c_driver(tpd_i2c_driver);
