...
const struct vendor_map g_vendor_map[] = {
	...
	{ 0x2ae1, "passive_pen", passive_pen_fw, sizeof(passive_pen_fw),
	  0x271F },
};

//#define ELAN_FW_PAGESIZE	(132)

static int get_hid_iap_ack(struct elan_ts_data *ts, uint8_t *cmd, int len,
			   uint8_t *buf, int rlen)
{
	...
	if (err != len) {
		dev_err(&ts->client->dev,
			"[elan] write page finish command fauled\n");
		return err;
	}
	...
	if (err == rlen) {
		if (memcmp(buf + 4, ack_ok, sizeof(ack_ok)) == 0) {
			dev_info(&ts->client->dev,
				 "[elan] iap write page response ok\n");
			return 0;
		} else if (memcmp(buf + 4, ack_rewrite, sizeof(ack_rewrite))) {
			dev_err(&ts->client->dev,
				"[elan] iap rewrite page response buf[4]:buf[5]= %x:%x\n",
				buf[4], buf[5]);
			return 1;
		} else {
			dev_err(&ts->client->dev,
				"[elan] iap ack  error  buf[4]:buf[5]= %x:%x\n",
				buf[4], buf[5]);
			return -1;
		}
	} else {
		dev_err(&ts->client->dev,
			"[elan] recv ack return value error\n");
		return err;
	}
...
{
	int len = 0;
	uint8_t remarkid[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00, 0x03,
					  0x00, 0x06, 0x96, 0x80, 0x1F,
					  0x00, 0x00, 0x21 };
	uint8_t buf[67] = { 0 };
	u16 remark_id = 0;
	...
	if (len != sizeof(remarkid)) {
		dev_err(&ts->client->dev,
			"[elan]Send query remark id cmd failed!! len=%d", len);
		return -EINVAL;
	} else
		dev_err(&ts->client->dev,
			"[elan]Remark id write successfully!");
	...
	if (len != sizeof(buf)) {
		dev_err(&ts->client->dev,
			"[elan]Send Check Address Read Data error. len=%d",
			len);
		return -EINVAL;
	} else {
		remark_id = (buf[7] << 8) | buf[8];
		dev_err(&ts->client->dev, "[elan]Get Remark id = 0x%4x",
			remark_id);
		return remark_id;
	}
}
...
static int hid_fw_upgrade_init(struct elan_ts_data *ts)
{
	int err = 0;
	u16 remark_id;
	const uint8_t flash_key[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
						 0x03, 0x00, 0x04, 0x54,
						 0xc0, 0xe1, 0x5a };
	const uint8_t check_addr[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
						  0x03, 0x00, 0x01, 0x10 };
	const uint8_t isp_cmd[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					       0x03, 0x00, 0x04, 0x54,
					       0x00, 0x12, 0x34 };
	uint8_t buf[67] = { 0 };
	...
	if (err != UNKNOW_TYPE) {
		...
		if (ts->fw_info.fw_bcl >= 0x60) {
			...
			if (ts->update_info.remark_id !=
			    (ts->update_info.remark_id & remark_id)) {
				dev_err(&ts->client->dev,
					"[elan]Remark id failed,exit update");
				return err;
			}
		}
	}
	...
	if (err != sizeof(flash_key)) {
		dev_err(&ts->client->dev,
			"[elan]send flash key failed ,exit update\n");
		return err;
	}
	...
	if (ts->recover == COMPARE_UPGRADE) { //normal mode
		...
		if (err != sizeof(isp_cmd)) {
			dev_err(&ts->client->dev,
				"[elan] send isp cmd failed, exit update\n");
			return err;
		}
	}
	...
	if (err != sizeof(check_addr)) {
		dev_err(&ts->client->dev,
			"[elan] send check addr failed, exit update\n");
		return err;
	}
	...
	if (err != sizeof(buf)) {
		dev_err(&ts->client->dev,
			"[elan] recv check addr response failed\n");
		return err;
	}

	dev_err(&ts->client->dev,
		"[elan] %s response_addr = 0x%02x, buf=0x%2x\n", __func__,
		response_addr, buf[4]);
	if (memcmp(&buf[4], &response_addr, 1)) {
		dev_err(&ts->client->dev,
			"[elan] response addr check failed,exit update\n");
		return err;
	} else {
...
{
	int ret = 0;
	uint8_t upgrade_end[HID_CMD_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x1A
	};
...
static int Hid_Fw_Update(struct i2c_client *client)
{
	...
	uint8_t qIapPagefinishcmd[HID_CMD_LEN] = { 0x04, 0x00, 0x23,
						   0x00, 0x03, 0x22 };
	uint8_t qIapWriteCmd[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00, 0x03,
					      0x21, 0x00, 0x00, 0x1c };
...
update_retry:
	...
	while (wFwSize) {
		wRemainLen = MIN((update->FwSize - wCount),
				 (IAP_FLASH_SIZE - (qOffset % IAP_FLASH_SIZE)));
		...
		memcpy(qIapWriteCmd + IAP_CMD_HEADER_LEN,
		       update->FwData + wCount, wCopyLen);
		qCmdLen += wCopyLen;
		...
		if ((qOffset % (IAP_FLASH_SIZE) == 0) ||
		    (wCount == update->FwSize)) {
			err = get_hid_iap_ack(ts, qIapPagefinishcmd,
					      sizeof(qIapPagefinishcmd),
	...
	if (err) {
		dev_err(&ts->client->dev,
			"[elan] fw upgrade finshed failed!!!\n");
		return -1;
	}
...
static int elan_fw_write_page(struct i2c_client *client, const void *page)
{
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	...
	for (retry = 0; retry < 5; retry++) {
		...
		if (error != FW_PAGE_SIZE) {
			dev_err(&client->dev, "IAP Write Page failed: %d\n",
				error);
			continue;
		}
		...
		if (error != sizeof(buf)) {
			dev_err(&client->dev, "IAP Ack read failed: %d\n",
				error);
			return error;
		}
		...
		error = -EIO;
		dev_err(&client->dev, "IAP Get Ack Error [%02x:%02x]\n", buf[0],
			buf[1]);
	}
	return error;
}
...
static int Normal_Fw_Update(struct i2c_client *client)
{
	...
	for (page = 0; page < update->PageNum; page++) {
		error = elan_fw_write_page(client, update->FwData +
							   page * FW_PAGE_SIZE);
		if (error) {
			dev_err(&client->dev,
...
{
	uint8_t flash_key[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					   0x03, 0x00, 0x04, CMD_W_PKT,
					   0xc0, 0xe1, 0x5a };
	uint8_t cal_cmd[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					 0x03, 0x00, 0x04, CMD_W_PKT,
					 0x29, 0x00, 0x01 };
	int err = 0;
	...
	err = ts->ops->poll();
	if (err) {
		dev_err(&client->dev, "[elan] %s: wait for int low failed %d\n",
			__func__, err);
		return err;
	} else {
		...
		if (err == sizeof(resp)) {
			...
			dev_err(&client->dev,
				"%s:%d recv calibrate data failed\n", __func__,
				__LINE__);
			return -EINVAL;
		}
	}
...
int elan_ts_normal_calibrate(struct i2c_client *client)
{
	...
	if (err) {
		...
		if (err == sizeof(resp)) {
			...
			dev_err(&client->dev,
				"%s:%d recv calibrate data failed", __func__,
				__LINE__);
			return -EINVAL;
		}
	}
...
{
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	const uint8_t check_rek_cmd[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
						     0x03, 0x00, 0x04, 0x53,
						     0xD0, 0x00, 0x01 };
	...
	if (err != sizeof(check_rek_cmd)) {
		dev_err(&client->dev,
			"[elan] %s send check rek command failed\n", __func__);
		return err;
	}
	...
	if (err != sizeof(resp)) {
		...
		dev_info(&client->dev,
			 "[elan] %s check rek resp 0x%2x:0x%2x:0x%2x:0x%2x\n",
			 __func__, resp[6], resp[7], resp[8], resp[9]);
...
static int elan_read_fw_from_sdcard(struct elan_ts_data *ts)
{
	...
	for (retry = 0; retry < 5; retry++) {
		...
		if (IS_ERR(firmware_fp)) {
			dev_err(&ts->client->dev,
				"[elan] retry to open user ekt file\n");
			mdelay(100);
		} else
			break;
	}
	...
	if (retry >= 5) {
		dev_err(&ts->client->dev, "[elan] open %s file error!!\n",
			update->fw_local_path);
		ret = -1;
		goto out_read_fw_from_user2;
	} else
		dev_dbg(&ts->client->dev, "[elan] open %s file sucess!!\n",
			update->fw_local_path);

	file_len = firmware_fp->f_path.dentry->d_inode->i_size;
	if (file_len == 0) {
		dev_dbg(&ts->client->dev, "[elan] Get File len err!!!!");
		ret = -2;
	...
	if (fw_data_user == NULL) {
		dev_err(&ts->client->dev, "[elan] malloc fw_data err\n");
		ret = -3;
		goto out_read_fw_from_user1;
	}

	ret = firmware_fp->f_op->read(firmware_fp, fw_data_user, file_len,
				      &firmware_fp->f_pos);
	if (ret != file_len) {
		dev_err(&ts->client->dev,
...
static int get_driver_fw(struct elan_ts_data *ts)
{
	...
	for (i = 0; i < vendor_num; i++) {
		dev_err(&ts->client->dev,
			...
			g_vendor_map[i].vendor_name, lcm_id,
			g_vendor_map[i].vendor_id);
...
int get_vendor_fw(struct elan_ts_data *ts, int type)
{
	...
	if (type == FROM_SYS_ETC_FIRMWARE) {
		...
		err = request_firmware(&p_fw_entry, update_info->FwName,
				       &ts->client->dev);
		if (err) {
			dev_err(&ts->client->dev,
			...
			dev_dbg(&ts->client->dev, "Firmware Size=%zu",
				p_fw_entry->size);
		...
		update_info->FwName = kasprintf(GFP_KERNEL, "elants_i2c.ekt");
		sprintf(update_info->fw_local_path, "%s%s", "/data/local/tmp",
			update_info->FwName);
		...
		if (err) {
			dev_err(&ts->client->dev,
				"Get FW Data From driver failed\n");
			return -1;
		}
	}
	update_info->remark_id = get_unaligned_le16(
		&update_info->FwData[update_info->FwSize - 4]);
	return err;
}
...
void check_update_flage(struct elan_ts_data *ts)
{
	...
	** support three methods to get fwdata.
	...
	if (err) {
		dev_err(&ts->client->dev,
			"[elan] ***counld not get fw,exit update flow!!***\n");
		goto exit_fw_upgrade;
	}

	if (ts->recover == FORCED_UPGRADE) {
		dev_err(&ts->client->dev,
			"[elan] *** fw is miss, force update fw!! ***\n");
		goto fw_upgrade;
	}
	...
	*/
	New_FW_ID = update_info->FwData[0X1FE67] << 8 |
		    update_info->FwData[0X1FE66];
	New_FW_VERSION =
		update_info->FwData[0x9F] << 8 | update_info->FwData[0x9E];

	dev_info(&ts->client->dev, "[elan] FW_ID=0x%4x,New_FW_ID=0x%4x\n",
		 fw_info->fw_id, New_FW_ID);
	dev_info(&ts->client->dev,
		 "[elan] FW_VERSION=0x%4x,New_FW_VERSION=0x%4x\n",
...
fw_upgrade:
	...
	for (; retry < 3; retry++) {
		...
		if (err)
			dev_err(&ts->client->dev,
				"[elan] After update fw get fw msg failed, retry=%d\n",
				retry);
		else
			break;
	}
	...
	for (retry = 0; retry < 3; retry++) {
		...
		if (err)
			dev_err(&ts->client->dev,
				"[elan] After update fw calibrate failed, retry=%d\n",
				retry);
		else {
			...
			if (err)
				dev_err(&ts->client->dev,
					"[elan] After update fw check rek failed, retry=%d\n",
					retry);
			else
				break;
		}
