			   ...
			   const char *buf, size_t count)
{
	...
	dev_info(&client->dev, "Reset Touch Screen Controller!\n");
	return count;
}
static DEVICE_ATTR(reset, S_IWUSR, NULL, store_reset);

static ssize_t show_gpio_int(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct elan_ts_data *ts = i2c_get_clientdata(client);
			    ...
			    char *buf)
{
	...
	return sprintf(
		buf,
		"FW VER = 0x%04x, FW ID = 0x%04x, BC VER = 0x%04x, tx:rx = %d:%d\n",
		ts->fw_info.fw_ver, ts->fw_info.fw_id, ts->fw_info.fw_bcv,
		ts->fw_info.tx, ts->fw_info.rx);
}
static ssize_t store_fw_info(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	int ret;
	struct i2c_client *client = to_i2c_client(dev);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "IAP STATUS = %s\n",
		       (ts->recover < 0 ?
				"UNKNOW" :
				(ts->recover == 0x01 ? "RECOVERY" : "NORMAL")));
}
static DEVICE_ATTR(iap_status, S_IWUSR | S_IRUSR, show_iap_status,
		   store_iap_status);
...
static ssize_t show_fw_store(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	...
	if (ts->fw_store_type > -1 && ts->fw_store_type < 3) {
		return sprintf(
			buf, "FW STORE = %s\n",
			(ts->fw_store_type == FROM_SYS_ETC_FIRMWARE ?
				 "/system/etc/firmware/elants_i2c.ekt" :
				 (ts->fw_store_type == FROM_SDCARD_FIRMWARE ?
					  "/data/local/tmp/elants_i2c.ekt" :
					  "build in driver")));
	} else {
...
static ssize_t store_fw_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	...
	if (type > -1 && type < 3) {
		...
		dev_info(&client->dev,
			 "[elan] fw store type out of range!!!\n");
	}
	return count;
}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "PRINT LEVEL = %s\n",
		       (ts->level == TP_DEBUG ?
				...
				(ts->level == TP_INFO ?
					 "INFO" :
					 (ts->level == TP_WARNING ? "WARNING" :
								    "ERROR"))));
}
static DEVICE_ATTR(tp_print_level, S_IWUSR | S_IRUSR, show_tp_print_level,
		   store_tp_print_level);
				 ...
				 size_t count)
{
	...
	sscanf(buf, "%d:%x:%x:%x:%x:%x:%x:%x:%x", &valid_size, (int *)&cmd[5],
	       (int *)&cmd[6], (int *)&cmd[7], (int *)&cmd[8], (int *)&cmd[9],
	       (int *)&cmd[10], (int *)&cmd[11], (int *)&cmd[12]);
	/*
	if (ts->irq_lock_flag == 0) {
		disable_irq(ts->hw_info.irq_num);
		ts->irq_lock_flag = 1;
...
static DEVICE_ATTR(raw_cmd, S_IWUSR | S_IRUSR, show_tp_cmd_recv,
		   store_tp_cmd_send);
...
static ssize_t show_swapcoord(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct elan_ts_data *ts = i2c_get_clientdata(client);

	return sprintf(buf, "swapbit = %d\n", ts->elan_swapbit);
}
static DEVICE_ATTR(swapcoord, S_IWUSR | S_IRUSR, show_swapcoord,
		   store_swapcoord);

static ssize_t ts_gesture_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	unsigned int flag;
	...
	if (!g_is_lcd_suspend) {
		...
		printk("lcd is suspend,not allowed to change gesture_mode\n");
	}
...
static DEVICE_ATTR(gesture_switch, 0664, ts_gesture_show, ts_gesture_store);

static struct attribute *elan_default_attributes[] = {
	&dev_attr_enable_irq.attr, &dev_attr_disable_irq.attr,
	&dev_attr_gpio_int.attr, &dev_attr_reset.attr, &dev_attr_calibrate.attr,
	&dev_attr_check_rek.attr, &dev_attr_fw_info.attr,
	&dev_attr_iap_status.attr, &dev_attr_fw_update.attr,
	&dev_attr_fw_store.attr,
	//	&dev_attr_tp_module_test.attr,
	&dev_attr_tp_print_level.attr, &dev_attr_raw_cmd.attr,
	&dev_attr_swapcoord.attr, &dev_attr_gesture_switch.attr, NULL
};
...
int elan_sysfs_attri_file(struct elan_ts_data *ts)
{
	...
	err = sysfs_create_group(&ts->client->dev.kobj,
				 &elan_default_attribute_group);
	if (err) {
		dev_err(&ts->client->dev,
			"[elan] %s sysfs create group error\n", __func__);
	} else {
		dev_err(&ts->client->dev,
			"[elan] %s sysfs create group success\n", __func__);
	}
