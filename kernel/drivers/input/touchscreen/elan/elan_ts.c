...
EXPORT_SYMBOL_GPL(g_is_lcd_suspend);
//define private data
...
{
	//	struct elan_ts_data *ts = elan_private_ts;

	dev_info(&ts->client->dev,
		"[elan] %s enter, irq = %d, on = %d, irq status=%s\n", __func__,
		ts->hw_info.irq_num, on,
		ts->irq_disabled == true ? "disable" : "enable");
...
{
	int status = 0, retry = 250; //20;

	do {
		status = gpio_get_value(elan_private_ts->hw_info.intr_gpio);
		if (status == 0)
			break;
		retry--;
		mdelay(5); //mdelay(20);
	...
	if (status > 0)
		dev_info(&elan_private_ts->client->dev,
...
static int elan_i2c_send(const uint8_t *buf, int count)
{
	int ret = -1;
	int retries = 0;
	struct i2c_client *client = elan_private_ts->client;
...
{
	uint8_t checkstatus[HID_CMD_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x18
	};
...
RETRY:
	...
	if (err) {
		dev_err(&client->dev, "[elan] ERROR: %s INT status high",
			__func__);
	...
	if (err != sizeof(buf)) {
		dev_err(&client->dev, "[elan] ERROR:%s Read Hello Data error\n",
			__func__);
		if ((retry--) > 0)
			goto RETRY;
		return -1;
	...
	dev_info(&client->dev, "[elan] FW Mode = 0x%2x\n", buf[4]);
...
static int get_normal_hello(struct i2c_client *client)
{
	int err = 0;
	uint8_t buf[8] = { 0 };
	uint8_t normal_hello[4] = { NORMAL_FW_NORMAL_MODE,
				    NORMAL_FW_NORMAL_MODE,
				    NORMAL_FW_NORMAL_MODE,
				    NORMAL_FW_NORMAL_MODE };
	uint8_t recovery_hello[4] = { NORMAL_FW_NORMAL_MODE,
				      NORMAL_FW_NORMAL_MODE,
				      NORMAL_FW_RECOVERY_MODE,
				      NORMAL_FW_RECOVERY_MODE };
	struct elan_ts_data *ts = i2c_get_clientdata(client);
...
static int hid_fw_packet_handler(struct i2c_client *client)
{
	...
	const uint8_t cmd_ver[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					       0x03, 0x00, 0x04, 0x53,
					       0x00, 0x00, 0x01 };
	const uint8_t cmd_id[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					      0x03, 0x00, 0x04, 0x53,
					      0xf0, 0x00, 0x01 };
	const uint8_t cmd_bc[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					      0x03, 0x00, 0x04, 0x53,
					      0x10, 0x00, 0x01 };
	const uint8_t cmd_osr[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					       0x03, 0x00, 0x04, 0x53,
					       0xD6, 0x00, 0x01 };
	const uint8_t cmd_test_ver[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
						    0x03, 0x00, 0x04, 0x53,
						    0xe0, 0x00, 0x01 };
	const uint8_t cmd_whck_ver[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
						    0x03, 0x00, 0x04, 0x53,
						    0xd2, 0x00, 0x01 };
	const uint8_t cmd_res[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00, 0x03,
	...
	/*fw version*/
	err = elan_ts_get_data(client, cmd_ver, sizeof(cmd_ver), rbuf,
			       sizeof(rbuf));
	if (err) {
	...
	/*fw id*/
	err = elan_ts_get_data(client, cmd_id, sizeof(cmd_id), rbuf,
			       sizeof(rbuf));
	if (err) {
	...
	err = elan_ts_get_data(client, cmd_bc, sizeof(cmd_bc), rbuf,
			       sizeof(rbuf));
	if (err) {
	...
	/*get finger osr*/
	err = elan_ts_get_data(client, cmd_osr, sizeof(cmd_osr), rbuf,
			       sizeof(rbuf));
	if (err) {
	...
	/*get trace num*/
	err = elan_ts_get_data(client, cmd_res, sizeof(cmd_res), rbuf,
			       sizeof(rbuf));
	if (err) {
		dev_err(&client->dev, "[elan] %s get finger osr failed\n",
			__func__);
	...
	/*get test ver*/
	err = elan_ts_get_data(client, cmd_test_ver, sizeof(cmd_test_ver), rbuf,
			       sizeof(rbuf));
	if (err) {
	...
	/*get whck ver*/
	err = elan_ts_get_data(client, cmd_whck_ver, sizeof(cmd_whck_ver), rbuf,
			       sizeof(rbuf));
	if (err) {
		dev_err(&client->dev, "[elan] %s get test ver failed\n",
			__func__);
	...
	dev_info(&client->dev, "[elan] %s fw version:0x%4.4x\n", __func__,
		 fw_info->fw_ver);
	dev_info(&client->dev, "[elan] %s fw id:0x%4.4x\n", __func__,
		 fw_info->fw_id);
	dev_info(&client->dev,
		 "[elan] %s bootcode version:0x%4.4x: low byte 0x%2.2x\n",
		 __func__, fw_info->fw_bcv, fw_info->fw_bcl);
	dev_info(&client->dev, "[elan] %s fw_info->rx, fw_info->tx: %d:%d\n",
		 __func__, fw_info->rx, fw_info->tx);
	dev_info(&client->dev,
		 "[elan] %s finger x/y resolution:0x%4.4x/0x%4.4x\n", __func__,
		 fw_info->finger_xres, fw_info->finger_yres);
	dev_info(&client->dev, "[elan] %s pen x/y resolution:0x%4.4x/0x%4.4x\n",
...
static int normal_fw_packet_handler(struct i2c_client *client)
{
...
#ifndef TWO_LAYER
	err = elan_ts_get_data(client, cmd_x, sizeof(cmd_x), rbuf,
			       sizeof(rbuf));
	if (err) {
		dev_err(&client->dev,
			"[elan] %s get finger xresolution failed\n", __func__);
		return err;
	}
	minor = ((rbuf[2])) | ((rbuf[3] & 0xf0) << 4);
	fw_info->finger_xres = minor;

	err = elan_ts_get_data(client, cmd_y, sizeof(cmd_y), rbuf,
			       sizeof(rbuf));
	if (err) {
		dev_err(&client->dev,
			"[elan] %s get finger yresolution failed\n", __func__);
		return err;
	}
	minor = ((rbuf[2])) | ((rbuf[3] & 0xf0) << 4);
	fw_info->finger_yres = minor;
#else
	err = elan_ts_get_data(client, cmd_info, sizeof(cmd_info), rbuf,
			       sizeof(rbuf));
	if (err) {
		dev_err(&client->dev,
			"[elan] %s get two layer x/y resolution failed\n",
			__func__);
		return err;
	}
	...
	dev_info(&client->dev, "[elan] %s fw id:0x%4.4x\n", __func__,
		 fw_info->fw_id);
	dev_info(&client->dev, "[elan] %s bootcode version:0x%4.4x\n", __func__,
		 fw_info->fw_bcv);
	dev_info(&client->dev,
		 "[elan] %s finger x/y resolution:0x%4.4x/0x%4.4x\n", __func__,
		 fw_info->finger_xres, fw_info->finger_yres);
...
static int elan_ts_recv_data(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	if (ts->chip_type == HID_TYPE_PROTOCOL) {
		if (rbuf[2] == HID_FID) {
			...
			if (rbuf[62] > 5) {
				...
				if (rc != sizeof(rbuf)) {
					dev_err(&ts->client->dev,
						"[elan] recv second report data error!!\n");
					return -1;
				}
		...
		if (rbuf[0] == NOR2_FID) {
			...
			report->finger.freport_idx = 3;
			report->finger.fbits =
				((rbuf[2] & 0x30) << 4) | (rbuf[1]);
		}
		report->finger.fbutton_value =
			rbuf[report->finger.fbuf_valid_size - 1];
		report->finger.fshift_byte = 3;
		report->tool_type = ELAN_FINGER;
		memcpy(buf, rbuf, report->finger.fbuf_valid_size);
	}

	if (rbuf[2] == GESTURE_HEAD) {
	...
	} else if (report->tool_type == ELAN_FINGER) {
...
static inline int elan_ts_fparse_xy(uint8_t *data, uint16_t *x, uint16_t *y,
				    const int type)
{
	*x = *y = 0;
...
static inline int elan_ts_pparse_xy(uint8_t *data, uint16_t *x, uint16_t *y,
				    uint16_t *p)
{
	*x = *y = *p = 0;
...
{
	struct input_dev *idev = ts->finger_idev;
	if (buf == NULL)
		return;
	...
	default:
		dev_err(&ts->client->dev,
			"[elan] %s undefine gesture packet %02x\n", __func__,
			buf[3]);
		break;
	}
}
	...
	if (report->tool_type == ELAN_FINGER) {
		for (reportid = 0; reportid < finger.fvalid_num; reportid++) {
			if (finger.fid == HID_FID) { /*hid over i2c protocol*/
				...
				if (fbits) {
					fbit = (((buf[finger.freport_idx] &
						  0xfc) >>
						 2) -
						1);
					elan_ts_fparse_xy(
						&buf[finger.freport_idx], &y,
						&x, finger.fid);

					x = 800 * x / 2112;
					y = 1200 * y / 3392;

					input_report_key(ts->finger_idev,
							 BTN_TOUCH, 1);
					input_report_key(ts->finger_idev,
							 BTN_TOOL_FINGER, true);
					input_report_abs(ts->finger_idev,
							 ABS_MT_POSITION_X, x);
					input_report_abs(ts->finger_idev,
							 ABS_MT_POSITION_Y, y);
					input_report_abs(ts->finger_idev,
		...
		if (!valid_num) {
			...
			input_report_key(ts->finger_idev, BTN_TOOL_FINGER,
					 false);
			input_mt_sync(ts->finger_idev);
		}
		...
		print_log(
			ts->level,
			"[elan] stylus.tip_status  %d, stylus.inrange_status %d\n",
...
static void elants_slot_report(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	if (report->tool_type == ELAN_FINGER) {
		if (finger.fid == HID_FID) { /*hid over i2c protocol*/
			...
			for (reportid = 0; reportid < finger.fvalid_num;
			     reportid++) {
				active = (buf[finger.freport_idx] & 0x03);
				id = (((buf[finger.freport_idx] & 0xfc) >> 2) -
				      1);
				elan_ts_fparse_xy(
					&buf[finger.freport_idx], &x, &y,
					finger.fid); //lcm x :y = 720 : 1280 tp x: y = 1296:720
				//dev_info(&ts->client->dev, "[elan] finger Raw X:Y:P====%d:%d:%d\n",x,y,p);
			#if defined(CONFIG_TOUCHSCREEN_DIRECTION_HORIZONTAL)
				x = ts->hw_info.screen_x * x /
				    ts->fw_info.finger_xres;
				y = ts->hw_info.screen_y * y /
				    ts->fw_info.finger_yres;
				...
				#else
					input_report_abs(ts->finger_idev,
							 ABS_MT_POSITION_X, y - 960);
					input_report_abs(ts->finger_idev,
							 ABS_MT_POSITION_Y, x);
				#endif
					dev_dbg(&ts->client->dev, "[elan] finger X:Y ====%d:%d\n",x,y);
				} else { /*finger leave*/
					input_mt_slot(ts->finger_idev, id);
					//input_report_key(ts->finger_idev, BTN_TOUCH, 0);
					input_mt_report_slot_state(
			...
			if (fbits || fprebits) {
				for (reportid = 0; reportid < finger.fvalid_num;
					...
					if (fbits & 0x01) {
						elan_ts_fparse_xy(
							&buf[finger.freport_idx],
							&y, &x, finger.fid);
						input_mt_slot(ts->finger_idev,
							      reportid);
						input_mt_report_slot_state(
							ts->finger_idev,
							MT_TOOL_FINGER, true);
		...
		if (stylus.key > 0 || !pkey) {
			switch (stylus.key) {
			case 2:
				pkey = BTN_STYLUS; //BTN_TOOL_RUBBER;
				input_report_key(ts->pen_idev, pkey, 1);
				input_sync(ts->pen_idev);
				break;
			case 3:
				...
				break;
			case 4:
				pkey = BTN_TOOL_RUBBER; //BTN_STYLUS;
				input_report_key(ts->pen_idev, pkey, 1);
				input_sync(ts->pen_idev);
				break;
			case 8:
		...
		print_log(
			ts->level,
			"[elan] stylus.inrange_status = %d, stylus.barrel_tip = %d\n",
			stylus.inrange_status, stylus.barrel_tip);
		if (stylus.inrange_status) {
			// elan_ts_pparse_xy(&buf[0],&x,&y,&p);
			//dev_info(&ts->client->dev, "[elan] pen Raw X:Y:P====%d:%d:%d\n",x,y,p);
			...
			input_mt_report_slot_state(ts->pen_idev, MT_TOOL_PEN,
						   true);
			input_report_key(ts->pen_idev, BTN_TOUCH, 1);
			input_report_abs(ts->pen_idev, ABS_MT_TOOL_TYPE,
					 MT_TOOL_PEN);
			...
			dev_dbg(&ts->client->dev, "[elan] pen X:Y:P:TX:TY ====%d:%d:%d:%d:%d\n",x,y,p,x_tilt,y_tilt);
		} else {
			input_mt_slot(ts->pen_idev, 0);
			input_mt_report_slot_state(ts->pen_idev, MT_TOOL_PEN,
						   false);
...
static void elan_ts_hid_report(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	if ((button != 0 && button != 0xFF) ||
	    (pbutton != 0 && pbutton != 0xFF) || (prekey != 0)) {
	...
	if (GESTURE_HEAD == buf[2]) {
		elan_gesture_report(ts, buf);
		return;
	}
...
static void elan_ts_report_data(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	default:
		dev_err(&ts->client->dev,
			"[elan] unknow type 0x%2x:0x%2x:0x%2x:0x%2x", buf[0],
			buf[1], buf[2], buf[3]);
		break;
	}
}
...
static int elan_request_pen_input_dev(struct elan_ts_data *ts)
{
	...
	if (ts->report_type == PROTOCOL_TYPE_B) {
		...
		input_set_abs_params(ts->pen_idev, ABS_MT_TOOL_TYPE, 0,
				     MT_TOOL_MAX, 0, 0);
		input_set_abs_params(
			ts->pen_idev, ABS_MT_POSITION_X, 0,
			ts->fw_info.pen_xres /*ts->hw_info.screen_x*/, 0, 0);
		input_set_abs_params(
			ts->pen_idev, ABS_MT_POSITION_Y, 0,
			ts->fw_info.pen_yres /*ts->hw_info.screen_y*/, 0, 0);
		input_set_abs_params(ts->pen_idev, ABS_MT_PRESSURE, 0, 4096, 0,
				     0);
	...
	ts->pen_idev->evbit[0] |=
		BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS) | BIT_MASK(EV_SYN);
...
static int elan_request_finger_input_dev(struct elan_ts_data *ts)
{
	...
	if (ts->report_type == PROTOCOL_TYPE_B) {
		...
		input_mt_init_slots(ts->finger_idev, FINGERS_NUM,
				    INPUT_MT_DIRECT);
		input_set_abs_params(ts->finger_idev, ABS_MT_TOOL_TYPE, 0,
				     MT_TOOL_MAX, 0, 0);
	} else {
	...
	input_set_abs_params(ts->finger_idev, ABS_MT_POSITION_X, 0,
			     ts->hw_info.screen_x, 0, 0);
	input_set_abs_params(ts->finger_idev, ABS_MT_POSITION_Y, 0,
			     ts->hw_info.screen_y, 0, 0);
...
static int elan_ts_register_interrupt(struct elan_ts_data *ts)
{
	...
	err = request_threaded_irq(ts->hw_info.irq_num, NULL,
				   elan_ts_irq_handler,
...
static void elan_ic_init_work(struct work_struct *work)
{
	...
	/*elan irq resgister*/
	rc = elan_ts_register_interrupt(elan_private_ts);
	if (rc) {
		dev_err(&elan_private_ts->client->dev,
...
static int elan_ts_setup(struct elan_ts_data *ts)
{
	...
	if (err < 0) {
		...
		dev_err(&ts->client->dev, "[elan] %s,ic status = %s", __func__,
			err == FORCED_UPGRADE ? "recovery" : "normal");
	}
...
{
	dev_info(&elan_private_ts->client->dev, "%s enter", __func__);

	filp->private_data = NULL;
	//elan_private_ts->user_handle_irq = 0;
	//elan_private_ts->int_val = 0;
	return 0;
}
			      ...
			      loff_t *offp)
{
	...
	if (ret != count) {
		dev_err(&client->dev,
			"[elan]elan elan_i2c_send fail, ret=%d \n", ret);
	}
			   ...
			   unsigned long arg)
{
	...
	case IOCTL_IAP_MODE_LOCK:
		if (elan_private_ts->power_lock == 0) {
			elan_private_ts->power_lock = 1;
			elan_switch_irq(ts, 0);
		}
		break;
	case IOCTL_IAP_MODE_UNLOCK:
		if (elan_private_ts->power_lock == 1) {
			elan_private_ts->power_lock = 0;
			elan_switch_irq(ts, 1);
		}
		break;
	case IOCTL_CHECK_RECOVERY_MODE:
		return elan_private_ts->recover;
		break;
...
static unsigned int elan_iap_poll(struct file *filp,
...
static void elan_touch_node_init(struct elan_ts_data *ts)
{
	...
	ts->p = proc_create("elan-iap", 0664, NULL,
			    (const struct proc_ops *)&elan_touch_fops);
	if (ts->p == NULL)
		dev_err(&ts->client->dev,
			"[elan error] proc_create failed!!\n");
	else
...
static int elan_power_initial(struct elan_ts_data *ts)
{
	...
	if (IS_ERR(ts->vdd)) {
		ret = PTR_ERR(ts->vdd);
		dev_err(&ts->client->dev,
			"elan Regulator get failed vdd rc=%d\n", ret);
		return ret;
	}
	...
	if (regulator_count_voltages(ts->vdd) > 0) {
		...
		if (ret) {
			dev_err(&ts->client->dev,
				"elan Regulator set_vtg failed vdd rc=%d\n",
				ret);
			goto reg_vdd_put;
		}
	}
...
*/
	...
	chip_hw_info->intr_gpio =
		of_get_named_gpio_flags(np, "elan,irq-gpio", 0, NULL);
	if (!gpio_is_valid(chip_hw_info->intr_gpio)) {
		dev_err(&ts->client->dev,
			"[elan] hw_info->intr_gpio invalid\n");
		ret = -EINVAL;
		goto request_intr_gpio_failed;
	}

	/*get reset gpio from dts*/
	chip_hw_info->rst_gpio =
		of_get_named_gpio_flags(np, "elan,rst-gpio", 0, NULL);
	...
	if (ret == 0) {
		ts->chip_type = data;
		dev_info(&ts->client->dev, "[elan]:chip protocol_type=%s",
			 ts->chip_type == 1 ? "HID IIC" : "NORMAL IIC");
	} else {
		ret = -EINVAL;
	...
	if (ret == 0) {
		ts->report_type = data;
		dev_info(&ts->client->dev, "[elan]:report protocol_type=%s",
			 ts->report_type == 1 ? "B protocol" : "A protocol");
	} else {
		ret = -EINVAL; //hw_info->rst_gpio;
		goto read_report_type_failed;
	}
	ts->hw_info.screen_x = 1600; // 1728;
	ts->hw_info.screen_y = 2560; //2368;
#if 0
	/*get lcm coordinate*/
...
static int elan_ts_hw_initial(struct elan_ts_data *ts)
{
	...
	//ts->irq_lock_flag = 0;
#if defined(CONFIG_TOUCHSCREEN_DIRECTION_HORIZONTAL)
	ts->elan_swapbit = 0x60; // high byte for pen, low byte for finger
#else
	ts->elan_swapbit = 0x30; // high byte for pen, low byte for finger
#endif
	//ts->hw_info = *hw_info;
			 ...
			 const struct i2c_device_id *id)
{
	...
	ts = devm_kzalloc(&client->dev, sizeof(struct elan_ts_data),
			  GFP_KERNEL);
	if (!ts) {
		dev_err(&client->dev, "%s: allocate elan_ts_data failed",
			__func__);
		return -ENOMEM;
	}
...
#ifdef SM_BUS
	if (i2c_smbus_xfer(client->adapter, client->addr, 0, I2C_SMBUS_READ, 0,
			   I2C_SMBUS_BYTE, &dummy) < 0) {
	...
	if (COMPARE_UPGRADE == ts->recover) {
		for (retry = 0; retry < 3; retry++) {
			err = elan_ts_check_calibrate(
				ts->client); /*ic reponse rek count,count != 0xff? "ok":"failed" */
			if (err) {
				dev_err(&ts->client->dev,
					"[elan] check rek failed, retry=%d\n",
					retry);
				err = elan_ts_calibrate(ts->client);
				if (err) {
					dev_err(&ts->client->dev,
						"[elan]calibrate failed, retry=%d\n",
						retry);
				} else
					break;
			} else
...
{
	uint8_t hid_cmd[37] = { 0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
				0x04, 0x53, 0x40, 0x00, 0x01 };
...
TRY:
	...
	if (ret != sizeof(hid_cmd)) {
		dev_err(&client->dev,
			"[elan] %s send check gesture mode failed %d\n",
			__func__, ret);
		if (retry--)
			goto TRY;
	} else {
		ret = ts->ops->poll();
		if (ret < 0) {
			dev_err(&client->dev, "[elan] polling int low failed %s\n",
				__func__);
			goto EXIT;
		}
		...
		if (ret != sizeof(rbuf)) {
			dev_err(&client->dev,
				"[elan] %s recv check gesture mode data failed %d\n",
				__func__, ret);
			goto EXIT;
		} else {
			...
			dev_info(
				&client->dev,
				"[elan] %s enter %d(rbuf[6]=1--gesture, rbuf[6]=0--normal) mode\n",
				__func__, rbuf[6]);
			ret = rbuf[6];
		}
	}
...
static int elan_gesture_enter_check(struct i2c_client *client, const int state)
{
	...
	TRY:
		...
		if (ret != sizeof(hid_cmd)) {
			dev_err(&client->dev,
				"[elan] %s enter  gesture mode failed %d\n",
				__func__, ret);
			if (retry--)
				goto TRY;
		} else {
			...
			if (state == ret) {
				dev_info(
					&client->dev,
					"[elan] %s enter %d(ret=1--gesture, ret=0--normal) mode\n",
					__func__, ret);
				elan_switch_irq(ts, 1);
				return 0;
			}
...
static void elan_release_point(void)
{
	...
	if (elan_private_ts->finger_idev && elan_private_ts->pen_idev) {
		fidev = elan_private_ts->finger_idev;
		pidev = elan_private_ts->pen_idev;

		if (elan_private_ts->report_type == PROTOCOL_TYPE_B) {
			for (i = 0; i < 10; i++) {
				input_mt_slot(fidev, i);
				input_mt_report_slot_state(fidev,
							   MT_TOOL_FINGER, 0);
			}

			if (elan_private_ts->report.tool_type == ELAN_PEN) {
				input_mt_slot(pidev, 0);
				input_mt_report_slot_state(pidev, MT_TOOL_PEN,
							   false);
			}
		...
		dev_err(&elan_private_ts->client->dev, "Noting done\n");
	}
	return;
}
...
static int elan_ts_set_power_state(struct i2c_client *client, int state)
{
	...
	uint8_t hid_cmd[HID_CMD_LEN] = { 0x04, 0x00, 0x23, 0x00,
					 0x03, 0x00, 0x04, CMD_W_PKT,
					 0x50, 0x00, 0x01 };
...
static int elan_ts_suspend(struct device *dev)
{
	...
	if (ts->power_lock == 0) {
		if (ts->gesture_mode == 1) {
			dev_err(&client->dev, "[elan] %s gesture flow \n",
				__func__);
			if (enable_irq_wake(ts->hw_info.irq_num)) {
				dev_err(&elan_private_ts->client->dev,
					"%s enable_irq_wake(irq:%d) fail",
					__func__, ts->hw_info.irq_num);
			}

			err = elan_gesture_enter_check(ts->client, 0x01);
			if (err == 0) {
				dev_err(&ts->client->dev,
					"%s gesture mode enter OK\n", __func__);
			}
		} else {
			...
			elan_switch_irq(ts, 0);
		try_set_power
			: // if system would not power off, must do this and check
			err = elan_ts_set_power_state(ts->client,
						      PWR_STATE_DEEP_SLEEP);
			if (err) {
				dev_err(&client->dev,
					"[elan] set power stats failed err = %d\n",
					err);
				if ((retry--) > 0)
					goto try_set_power;
			}
	...
	if (ts->power_lock == 0) {
		if (ts->gesture_mode == 1) {
			if (disable_irq_wake(ts->hw_info.irq_num)) {
				dev_err(&ts->client->dev,
					"%s disable_irq_wake(irq:%d) fail",
					__func__, ts->hw_info.irq_num);
			}
			elan_ts_hw_reset(&ts->hw_info);
		} else {
			dev_err(&client->dev,
				"[elan] reset gpio to resum tp\n");
			/*device power on*/
...
static const struct i2c_device_id elan_ts_id[] = { { ELAN_TS_NAME, 0 }, {} };
