...
static int gc8034_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		// Re-init global reg here,in case rkaiq_3A_server call start_stream directly and dosen't call gc8034_s_power
		ret = gc8034_write_array(gc8034->client, gc8034->cur_mode->global_reg_list);
		if (ret) {
