...
#define IMX214_GAIN_MIN			0x10
#define IMX214_GAIN_MAX			0x140
#define IMX214_GAIN_STEP		0x1
#define IMX214_GAIN_DEFAULT		0x20
...
#define IMX214_FETCH_AGAIN_H(VAL)      (((VAL) >> 8) & 0x01)
#define IMX214_FETCH_AGAIN_L(VAL)      ((VAL) & 0xFF)
...
static const struct regval imx214_global_regs[] = {
	{0x0136, 0x18},
	{0x0137, 0x00},
	{0x0101, 0x00},		//旋转角度，0x00 0°；0x03 180°   对应更改IMX214_MEDIA_BUS_FMT
	{0x0105, 0x01},
	{0x0106, 0x01},
	{0x4550, 0x02},
	{0x4601, 0x04},
...
static int imx214_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ANALOGUE_GAIN:
        /* gain_reg = 512 - 512 / gain_ana
         * manual multiple 16 to add accuracy:
         * then formula change to:
         * gain_reg = 512 - 512 * 16 / (gain_ana * 16)
         */
        if (ctrl->val > 0x140)
                ctrl->val = 0x140;
        if (ctrl->val < 0x10)
        ...
        again = 512 - 512 * 16 / ctrl->val;
        ret = imx214_write_reg(imx214->client, IMX214_REG_GAIN_H,
                               IMX214_REG_VALUE_08BIT,
                               IMX214_FETCH_AGAIN_H(again));
        ret |= imx214_write_reg(imx214->client, IMX214_REG_GAIN_L,
                                IMX214_REG_VALUE_08BIT,
                                IMX214_FETCH_AGAIN_L(again));
