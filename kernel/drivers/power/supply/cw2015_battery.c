...
#include "cw2015_battery_temperature.h"
...
int cw2015_get_battery_temperature(void)
{
	if (!g_cw_bat)
		return -1;

	return g_cw_bat->temperature * 10;
}
EXPORT_SYMBOL_GPL(cw2015_get_battery_temperature);

int cw2015_get_battery_capacity(void)
{
	if (!g_cw_bat)
		return -1;
...
EXPORT_SYMBOL_GPL(cw2015_get_battery_capacity);
...
static int binarySearch(struct tempval arr[], int low, int high, int target)
{
	// At this point, 'low' and 'high' indices indicate the elements
	// that are closest to the target resistance, but on opposite sides.
	int mid = low + (high - low) / 2;

	if (high < low)
		return low;

	if (arr[mid].resistance == target)
		return mid;
	else if (arr[mid].resistance > target)
		return binarySearch(arr, mid + 1, high, target);
	else
		return binarySearch(arr, low, mid - 1, target);
}

static int cw_battery_get_temperature(int resistance)
{
	int size = sizeof(temperature_data) / sizeof(temperature_data[0]);
	int index = binarySearch(temperature_data, 0, size - 1, resistance);
	int leftIndex = index;
	int rightIndex = index + 1;

	if (temperature_data[index].resistance == resistance)
		return temperature_data[index].temperature;

	if (abs(temperature_data[leftIndex].resistance - resistance) <=
		abs(temperature_data[rightIndex].resistance - resistance)) {
			return temperature_data[leftIndex].temperature;
		} else {
			return temperature_data[rightIndex].temperature;
		}
}

static int cw_update_temperature(struct cw_battery *cw_bat)
{
	int ret;
	int value = 900;
	int R1 = 10 * 10000;

	ret = iio_read_channel_processed(cw_bat->channel, &value);
	if (unlikely(ret < 0)) {
		dev_err(cw_bat->dev, "battery adc read error");
		cw_bat->temperature = 25;
	} else {
		R1 = (value * 10 * 10000) / (1800 - value);
		cw_bat->temperature = cw_battery_get_temperature(R1);
	}

	return cw_bat->temperature;
}
...
static void cw_bat_work(struct work_struct *work)
{
	...
	dev_dbg(cw_bat->dev, "temperature = %d C\n", cw_bat->temperature);
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = cw_bat->temperature * 10;
		break;
...
static int cw_bat_probe(struct i2c_client *client)
{
	...
	cw_bat->channel = devm_iio_channel_get(cw_bat->dev, "battery_adc");
	if (IS_ERR(cw_bat->channel)) {
		ret = PTR_ERR(cw_bat->channel);
		if (ret == -EPROBE_DEFER)
			return ret;	/* iio not ready */
		dev_warn(cw_bat->dev, "could not request battery_adc iio channel (%d)", ret);
		cw_bat->channel = NULL;
	}
