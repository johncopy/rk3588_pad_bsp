...
enum bq25700_fields {
	...
	OUTPUT_INPUT_VOL,
	OUTPUT_SYS_POWER, /*reg23h*/
	OUTPUT_DSG_CUR,
	OUTPUT_CHG_CUR, /*reg24h*/
	OUTPUT_INPUT_CUR,
	OUTPUT_CMPIN_VOL, /*reg25h*/
	OUTPUT_SYS_VOL,
	OUTPUT_BAT_VOL, /*reg26h*/
	...
	IDCHG_VTH,
	IDCHG_DEG,
	PROCHOT_PROFILE_COMP,
	PROCHOT_PROFILE_ICRIT,
	PROCHOT_PROFILE_INOM,
	PROCHOT_PROFILE_IDCHG,
	PROCHOT_PROFILE_VSYS,
	PROCHOT_PROFILE_BATPRES,
	PROCHOT_PROFILE_ACOK,
	/*reg34h*/
	ADC_CONV,
	ADC_START,
	ADC_FULLSCALE,
...
struct bq25700_device {
	...
	struct device_node *notify_node1;
	...
	struct delayed_work temp_work; // temperature det
	...
	struct gpio_desc *led_red_io;
	struct gpio_desc *led_green_io;
	...
	int pd_charge_only_typec0;
	int pd_charge_only_typec1;
...
static const struct regmap_range bq25700_volatile_reg_ranges[] = {
	regmap_reg_range(0x12, 0x12), regmap_reg_range(0x14, 0x15),
	regmap_reg_range(0x20, 0x26), regmap_reg_range(0x30, 0x35),
	regmap_reg_range(0x3B, 0x3F), regmap_reg_range(0xFE, 0xFF),
};
...
static int bq25700_fw_read_u32_props(struct bq25700_device *charger)
{
	...
	struct {
		...
		{ "ti,otg-voltage", false, TBL_OTGVOL, &init->otg_voltage },
		{ "ti,otg-current", false, TBL_OTGCUR, &init->otg_current },
	};
	...
	for (i = 0; i < ARRAY_SIZE(props); i++) {
		...
		if ((props[i].tbl_id == TBL_CHGMAX) &&
			...
			dev_err(charger->dev,
				"ti,max-charge-voltage is error\n");
			return -ENODEV;
		}
		...
		*props[i].conv_data =
			bq25700_find_idx(property, props[i].tbl_id);
...
static void bq25700_set_input_vibus(struct bq25700_device *charger,
				    u32 input_voltage, u32 input_current)
{
	bq25700_field_write(charger, INPUT_VOLTAGE, input_voltage);
	bq25700_field_write(charger, INPUT_CURRENT, input_current);
	bq25700_field_write(charger, CHARGE_CURRENT, charger->init_data.ichg);
}
					     ...
					     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		else if (state.in_fchrg == 1 || state.in_pchrg == 1)
	...
	case POWER_SUPPLY_PROP_HEALTH:
		if (!state.fault_acoc && !state.fault_acov &&
		    !state.fault_batoc)
...
static int bq25700_power_supply_init(struct bq25700_device *charger)
{
	...
	charger->supply_charger = power_supply_register(
		charger->dev, &bq25700_power_supply_desc, &psy_cfg);
...
static void bq25700_disable_typec0(struct bq25700_device *charger)
{
	if (!IS_ERR_OR_NULL(charger->typec0_enable_io))
		gpiod_direction_output(charger->typec0_enable_io, 0);
}

static void bq25700_disable_typec1(struct bq25700_device *charger)
{
	if (!IS_ERR_OR_NULL(charger->typec1_enable_io))
		gpiod_direction_output(charger->typec1_enable_io, 0);
}

static void bq25700_typec0_connect(struct bq25700_device *charger, u32 vol_idx,
				   u32 cur_idx)
{
	dev_info(charger->dev, "%s: typec0 status = %s\n", __func__,
		 usbsta[charger->typec0_status]);
	dev_info(charger->dev, "%s: typec1 status = %s\n", __func__,
		 usbsta[charger->typec1_status]);

	if (charger->typec1_status != USB_STATUS_NONE) {
		dev_info(charger->dev, "typec1 connected. Charging with typec1.\n");
	} else {
		msleep(SWITCH_DELAY_TIME);
		bq25700_set_input_vibus(charger, vol_idx, cur_idx);
		bq25700_enable_typec0(charger);

		if (!wake_lock_active(&charger->chrg_wl))
			wake_lock(&charger->chrg_wl);
	}
}

static void bq25700_typec1_connect(struct bq25700_device *charger, u32 vol_idx,
				   u32 cur_idx)
{
	dev_info(charger->dev, "%s: typec0 status = %s\n", __func__,
		 usbsta[charger->typec0_status]);
	...
	if (charger->typec0_status != USB_STATUS_NONE) {
		dev_info(charger->dev, "typec1 connected. Switch to typec1 charging.\n");
		bq25700_disable_typec0(charger);
		msleep(SWITCH_DELAY_TIME);
	}
...
static void bq25700_typec0_disconnect(struct bq25700_device *charger)
{
	dev_info(charger->dev, "%s: typec0 status = %s\n", __func__,
		 usbsta[charger->typec0_status]);
	dev_info(charger->dev, "%s: typec1 status = %s\n", __func__,
	...
	if (wake_lock_active(&charger->chrg_wl) &&
		...
		wake_unlock(&charger->chrg_wl);
}

static void bq25700_typec1_disconnect(struct bq25700_device *charger)
{
	u32 vol_idx, cur_idx;
	vol_idx = bq25700_find_idx(DEFAULT_INPUTVOL, TBL_INPUTVOL);

	charger->typec1_status = USB_STATUS_NONE;
	bq25700_disable_typec1(charger);
	...
	if(charger->typec0_status == USB_STATUS_NONE) {
		...
		dev_info(charger->dev, "typec1 disconnect. Charging with typec0.\n");
	}

	switch (charger->typec0_status) {
	case USB_STATUS_PD:
		vol_idx = bq25700_find_idx(charger->pd0_vol, TBL_INPUTVOL);
		cur_idx = bq25700_find_idx(charger->pd0_cur, TBL_INPUTCUR);

		bq25700_typec0_connect(charger, vol_idx, cur_idx);
		break;
	case USB_STATUS_AC:
		bq25700_typec0_connect(charger, vol_idx,
...
static void bq25700_pd_connect_handle(struct bq25700_device *bq,
				      struct power_supply *psy)
{
	...
	bq->pd0_chr_cur = prop.intval;
}

static void bq25700_pd_connect_handle1(struct bq25700_device *bq,
				      struct power_supply *psy)
{
	int ret;
	union power_supply_propval prop;
	...
	bq->pd1_chr_cur = prop.intval;
}

static void bq25700_pd_disconnect_handle(struct bq25700_device *charger,
					 struct power_supply *psy)
{
	int ret;
	union power_supply_propval prop;
	...
	/* Do nothing if USB in here */
	if (prop.intval == 1)
		return;

exit:
	if (psy->dev.parent->of_node == charger->notify_node)
		bq25700_typec0_disconnect(charger);
	else if (psy->dev.parent->of_node == charger->notify_node1)
		bq25700_typec1_disconnect(charger);
}
...
static void bq25700_pd_worker(struct work_struct *work)
{
	struct bq25700_device *charger = container_of(work,
						      struct bq25700_device,
						      pd_work.work);
	u32 vol_idx, cur_idx, chr_idx;
	struct bq25700_state state;

	vol_idx = bq25700_find_idx((charger->pd0_vol - 1280000 - 3200000), TBL_INPUTVOL);
	cur_idx = bq25700_find_idx(charger->pd0_cur, TBL_INPUTCUR);
	chr_idx = bq25700_find_idx(charger->pd0_chr_cur, TBL_ICHG);

	dev_info(charger->dev, "INPUT_VOLTAGE: %d -> %d\n", charger->pd0_vol, vol_idx);
	dev_info(charger->dev, "INPUT_CURRENT: %d -> %d\n", charger->pd0_cur, cur_idx);
	dev_info(charger->dev, "CHARGE_CURRENT: %d -> %d\n", charger->pd0_chr_cur, chr_idx);
...
static void bq25700_pd_worker1(struct work_struct *work)
{
	struct bq25700_device *charger = container_of(work,
						      struct bq25700_device,
						      pd_work1.work);
	...
	vol_idx = bq25700_find_idx((charger->pd1_vol - 1280000 - 3200000), TBL_INPUTVOL);
	cur_idx = bq25700_find_idx(charger->pd1_cur, TBL_INPUTCUR);
	chr_idx = bq25700_find_idx(charger->pd1_chr_cur, TBL_ICHG);

	dev_info(charger->dev, "INPUT_VOLTAGE: %d -> %d\n", charger->pd1_vol, vol_idx);
	dev_info(charger->dev, "INPUT_CURRENT: %d -> %d\n", charger->pd1_cur, cur_idx);
	dev_info(charger->dev, "CHARGE_CURRENT: %d -> %d\n", charger->pd1_chr_cur, chr_idx);

	charger->typec1_status = USB_STATUS_PD;

	bq25700_typec1_connect(charger, vol_idx, cur_idx);
...
{
	struct bq25700_device *bq = container_of(nb, struct bq25700_device, nb);
	...
	if (ret != 0)
		return NOTIFY_OK;
	/* online=0: USB out */
	if (prop.intval == 0 && (bq->typec0_status == USB_STATUS_PD)) {
		bq->pd_psy = psy;
	...
	if (ret != 0)
		return NOTIFY_OK;
	if (prop.intval > 0 && (prop.intval / 1000000 != bq->pd0_cur)) {
		bq25700_pd_connect_handle(bq, psy);
...
static int bq2570x_pd_notifier_call1(struct notifier_block *nb,
				     unsigned long val, void *v)
{
	struct bq25700_device *bq =
		container_of(nb, struct bq25700_device, nb1);
	struct power_supply *psy = v;
	union power_supply_propval prop;
	int ret;
	...
	if (bq->notify_node1) {
		if (!psy->dev.parent ||
		    psy->dev.parent->of_node != bq->notify_node1)
			return NOTIFY_OK;
	} else if (bq->plat_data.notify_device) {
		if (strcmp(psy->desc->name, bq->plat_data.notify_device) != 0)
			return NOTIFY_OK;
	...
	/* online=0: USB out */
	if (prop.intval == 0 && (bq->typec1_status == USB_STATUS_PD)) {
		bq->pd_psy1 = psy;
		queue_delayed_work(bq->usb_charger_wq, &bq->discnt_work1,
				   msecs_to_jiffies(10));
		return NOTIFY_OK;
	}
	...
	if (prop.intval > 0 && (prop.intval / 1000000 != bq->pd1_cur)) {
		bq25700_pd_connect_handle1(bq, psy);
		queue_delayed_work(bq->usb_charger_wq, &bq->pd_work1,
				   msecs_to_jiffies(10));
	}
	return NOTIFY_OK;
}
				       ...
				       struct extcon_dev *edev)
{
	...
	if (charger->typec0_status == USB_STATUS_PD &&
	    edev == charger->cable_edev)
		return;

	if (charger->typec1_status == USB_STATUS_PD &&
	    edev == charger->cable_edev_1)
		return;
	...
	if (extcon_get_state(edev, EXTCON_CHG_USB_SDP) > 0) {
		...
		ibus_input = charger->init_data.input_current_sdp;
	} else if (extcon_get_state(edev, EXTCON_CHG_USB_DCP) > 0) {
		charger_state = USB_TYPE_AC_CHARGER;
		ibus_input = charger->init_data.input_current_dcp;
	} else if (extcon_get_state(edev, EXTCON_CHG_USB_CDP) > 0) {
		charger_state = USB_TYPE_CDP_CHARGER;
		ibus_input = charger->init_data.input_current_cdp;
	} else {
		charger_state = USB_TYPE_UNKNOWN_CHARGER;
	}

	switch (charger_state) {
	case USB_TYPE_USB_CHARGER:
		charger->typec0_status = USB_STATUS_USB;
		bq25700_typec0_connect(charger, vol_idx, ibus_input);
		break;
	...
	case USB_TYPE_CDP_CHARGER:
		charger->typec0_status = USB_STATUS_CDP;
		bq25700_typec0_connect(charger, vol_idx, ibus_input);
		break;
...
static void bq25700_charger_usb_bc_handel(struct bq25700_device *charger)
{
	...
	case USB_BC_TYPE_SDP:
		bq25700_enable_charger(charger,
				       charger->init_data.input_current_sdp);
		dev_info(charger->dev, "USB_TYPE_USB_CHARGER\n");
		break;
	...
	case USB_BC_TYPE_CDP:
		bq25700_enable_charger(charger,
				       charger->init_data.input_current_cdp);
		dev_info(charger->dev, "USB_TYPE_CDP_CHARGER\n");
		break;
	default:
		break;
	}
...
static void bq25700_charger_evt_worker(struct work_struct *work)
{
	...
	if (charger->usb_bc == 0) {
		...
		bq25700_charger_evt_handel(charger, edev);
		bq25700_get_chip_state(charger, &state);
		charger->state = state;
...
{
	struct bq25700_device *charger =
		container_of(work, struct bq25700_device, usb_work1.work);
...
static void bq25700_discnt_evt_worker(struct work_struct *work)
{
	...
	mutex_lock(&charger->lock);
	bq25700_pd_disconnect_handle(charger, charger->pd_psy);
	bq25700_get_chip_state(charger, &state);
	mutex_unlock(&charger->lock);
	power_supply_changed(charger->supply_charger);
}
...
{
	struct bq25700_device *charger =
		container_of(work, struct bq25700_device, discnt_work1.work);
	struct bq25700_state state;

	mutex_lock(&charger->lock);
	bq25700_pd_disconnect_handle(charger, charger->pd_psy1);
	bq25700_get_chip_state(charger, &state);
	mutex_unlock(&charger->lock);
	power_supply_changed(charger->supply_charger);
}
				      ...
				      struct notifier_block *able_cg_nb)
{
	...
	ret = extcon_register_notifier(edev, EXTCON_CHG_USB_SDP, able_cg_nb);
	if (ret < 0) {
		dev_err(charger->dev, "failed to register notifier for SDP\n");
		return ret;
	}

	ret = extcon_register_notifier(edev, EXTCON_CHG_USB_DCP, able_cg_nb);
	if (ret < 0) {
		dev_err(charger->dev, "failed to register notifier for DCP\n");
		return ret;
	}

	ret = extcon_register_notifier(edev, EXTCON_CHG_USB_CDP, able_cg_nb);
	if (ret < 0) {
...
static int bq25700_register_cg_nb(struct bq25700_device *charger)
{
	...
	if (charger->usb_bc == 0) {
		if (charger->cable_edev && !charger->pd_charge_only_typec0) {
			/* Register chargers  */
		...
		if (charger->cable_edev_1 && !charger->pd_charge_only_typec1) {
		...
		if (ret) {
			dev_err(charger->dev,
				"failed to register notifier for bc\n");
			return -EINVAL;
		}
	}
	return 0;
...
static int bq25700_register_pd_nb(struct bq25700_device *charger)
{
	...
	if (charger->notify_node1) {
		INIT_DELAYED_WORK(&charger->discnt_work1,
				  bq25700_discnt_evt_worker1);
		INIT_DELAYED_WORK(&charger->pd_work1,
					bq25700_pd_worker1);
		charger->nb1.notifier_call = bq2570x_pd_notifier_call1;
		ret = power_supply_reg_notifier(&charger->nb1);
		if (ret) {
			dev_err(charger->dev, "failed to reg notifier1: %d\n",
				ret);
			return ret;
		}
	}
...
static long bq25700_init_usb(struct bq25700_device *charger)
{
	...
	if (bq25700_register_otg_vbus_regulator(charger) < 0) {
		dev_warn(charger->dev, "Cannot register otg vbus regulator\n");
...
extern int cw2015_get_battery_temperature(void);
static void bq25700_update_temp_status(struct work_struct *work)
{
	int temperature = cw2015_get_battery_temperature();
	struct bq25700_device *charger =
		container_of(work, struct bq25700_device, temp_work.work);
	struct device *dev = charger->dev;

	if (temperature <= 50 && charger->high_temp_mark == 1) {
		if (charger->typec1_status != USB_STATUS_NONE) {
			bq25700_enable_typec1(charger);
		} else if (charger->typec0_status != USB_STATUS_NONE) {
			bq25700_enable_typec0(charger);
		}
		charger->high_temp_mark = 0;
	} else if (temperature > 50 && charger->high_temp_mark == 0) {
		dev_err(dev, "Turn off charging when the temperature exceeds 50 degrees Celsius\n");
		if (charger->typec1_status != USB_STATUS_NONE) {
			bq25700_disable_typec1(charger);
		}
		if (charger->typec0_status != USB_STATUS_NONE) {
		...
		charger->high_temp_mark = 1;
	}

	queue_delayed_work(charger->usb_charger_wq, &charger->temp_work,
				msecs_to_jiffies(500));
}

static void bq25700_init_temp_det(struct bq25700_device *charger)
{
	INIT_DELAYED_WORK(&charger->temp_work, bq25700_update_temp_status);
	queue_delayed_work(charger->usb_charger_wq, &charger->temp_work,
				msecs_to_jiffies(1000));
}

extern int cw2015_get_battery_capacity(void);
static void bq25700_update_led_status(struct work_struct *work)
{
	int cap = cw2015_get_battery_capacity();
	struct bq25700_device *charger =
		container_of(work, struct bq25700_device, led_work.work);

	if (charger->typec0_status == USB_STATUS_NONE &&
		charger->typec1_status == USB_STATUS_NONE) {
		if (!IS_ERR_OR_NULL(charger->led_red_io) &&
			gpiod_get_value(charger->led_red_io) == 1)
			gpiod_direction_output(charger->led_red_io, 0);
		if (!IS_ERR_OR_NULL(charger->led_green_io) &&
			gpiod_get_value(charger->led_green_io) == 1)
			gpiod_direction_output(charger->led_green_io, 0);
	} else {
		if (cap < 100) {
			if (!IS_ERR_OR_NULL(charger->led_red_io) &&
				gpiod_get_value(charger->led_red_io) == 0)
				gpiod_direction_output(charger->led_red_io, 1);
			if (!IS_ERR_OR_NULL(charger->led_green_io) &&
				gpiod_get_value(charger->led_green_io) == 1)
				gpiod_direction_output(charger->led_green_io,
						       0);
		} else if (cap == 100) {
			if (!IS_ERR_OR_NULL(charger->led_red_io) &&
				gpiod_get_value(charger->led_red_io) == 1)
				gpiod_direction_output(charger->led_red_io, 0);
			if (!IS_ERR_OR_NULL(charger->led_green_io) &&
				gpiod_get_value(charger->led_green_io) == 0)
				gpiod_direction_output(charger->led_green_io, 1);
		}
	}

	queue_delayed_work(charger->usb_charger_wq, &charger->led_work,
				msecs_to_jiffies(500));
}

static void bq25700_init_led(struct bq25700_device *charger)
{
	INIT_DELAYED_WORK(&charger->led_work, bq25700_update_led_status);
	queue_delayed_work(charger->usb_charger_wq, &charger->led_work,
				msecs_to_jiffies(1000));
}
...
static int bq25700_parse_dt(struct bq25700_device *charger)
{
	...
	charger->typec0_enable_io = devm_gpiod_get_optional(
		charger->dev, "typec0-enable", GPIOD_IN);
	if (!IS_ERR_OR_NULL(charger->typec0_enable_io))
		gpiod_direction_output(charger->typec0_enable_io, 0);

	charger->typec1_enable_io = devm_gpiod_get_optional(
		charger->dev, "typec1-enable", GPIOD_IN);
	if (!IS_ERR_OR_NULL(charger->typec1_enable_io))
		gpiod_direction_output(charger->typec1_enable_io, 0);

	charger->typec0_discharge_io = devm_gpiod_get_optional(
		charger->dev, "typec0-discharge", GPIOD_IN);

	charger->typec1_discharge_io = devm_gpiod_get_optional(
		charger->dev, "typec1-discharge", GPIOD_IN);

	charger->otg_mode_en_io =
		devm_gpiod_get_optional(charger->dev, "otg-mode-en", GPIOD_IN);
	if (!IS_ERR_OR_NULL(charger->otg_mode_en_io))
		gpiod_direction_output(charger->otg_mode_en_io, 0);

	charger->led_red_io =
		devm_gpiod_get_optional(charger->dev, "led-red", GPIOD_IN);
	if (!IS_ERR_OR_NULL(charger->led_red_io))
		gpiod_direction_output(charger->led_red_io, 0);

	charger->led_green_io =
		devm_gpiod_get_optional(charger->dev, "led-green", GPIOD_IN);
	if (!IS_ERR_OR_NULL(charger->led_green_io))
		gpiod_direction_output(charger->led_green_io, 0);

	ret = of_property_read_u32(np, "pd-charge-only-typec0",
				   &charger->pd_charge_only_typec0);
	if (ret < 0)
		dev_err(charger->dev, "pd-charge-only-typec0!\n");

	ret = of_property_read_u32(np, "pd-charge-only-typec1",
				   &charger->pd_charge_only_typec1);
	if (ret < 0)
		dev_err(charger->dev, "pd-charge-only-typec1!\n");
	...
	if (np)
		charger->notify_node =
			of_parse_phandle(np, "ti,usb-charger-detection", 0);
	if (!charger->notify_node)
		dev_err(charger->dev,
			"No ti,usb-charger-detection(index 0) property!\n");

	if (np)
		charger->notify_node1 =
			of_parse_phandle(np, "ti,usb-charger-detection", 1);
	if (!charger->notify_node1)
		dev_err(charger->dev,
			"No ti,usb-charger-detection(index 1) property!\n");
			 ...
			 const struct i2c_device_id *id)
{
	...
	charger->typec0_status = USB_STATUS_NONE;
	charger->typec1_status = USB_STATUS_NONE;
	charger->typec0_tmp = USB_STATUS_NONE;
	charger->typec1_tmp = USB_STATUS_NONE;
	charger->pd0_vol = 0;
	charger->pd0_cur = 0;
	charger->pd0_chr_cur = 0;
	charger->pd1_vol = 0;
	charger->pd1_cur = 0;
	charger->pd1_chr_cur = 0;
	charger->high_temp_mark = 0;
	...
	if (!charger_np)
		charger_np =
			of_find_compatible_node(NULL, NULL, "southchip,sc8885");
	if (charger_np) {
		charger->regmap =
			devm_regmap_init_i2c(client, &bq25700_regmap_config);
		...
		for (i = 0; i < ARRAY_SIZE(bq25700_reg_fields); i++) {
			...
			charger->rmap_fields[i] = devm_regmap_field_alloc(
				dev, charger->regmap, reg_fields[i]);
		...
		charger->regmap =
			devm_regmap_init_i2c(client, &bq25703_regmap_config);
	...
	wake_lock_init(&charger->chrg_wl, WAKE_LOCK_SUSPEND, "bq25703_wl");
	...
	bq25700_init_led(charger);
	bq25700_init_temp_det(charger);
	...
	ret = devm_request_threaded_irq(dev, client->irq, NULL,
					...
					irq_flag | IRQF_ONESHOT, "bq25700_irq",
					charger);
	if (ret)
		goto irq_fail;
