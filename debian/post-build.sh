...
function add_build_info()
{
    ...
    # 从配置文件读取基础版本，如果文件不存在则使用默认值
    VERSION_FILE="${TARGET_DIR}/etc/rom-version"
    if [ -f "$VERSION_FILE" ]; then
        BASE_VERSION=$(cat "$VERSION_FILE" | tr -d '[:space:]')
        echo "Reading BASE_VERSION from $VERSION_FILE: $BASE_VERSION"
    else
        BASE_VERSION="1.0.0"
    ...
    ROM_VERSION="${BASE_VERSION}-${TIMESTAMP}"

    # 添加新的版本信息
    echo "ROM_VERSION=\"${ROM_VERSION}\"" >> "$OS_RELEASE"
}
