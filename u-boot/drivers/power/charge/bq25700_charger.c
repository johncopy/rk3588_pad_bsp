...
static void bq25700_irq_handler(int irq, void *data)
{
	printf(">>> ENTER %s.\n", __func__);
}

static int bq25700_irq_init(struct udevice *dev)
{
	struct bq25700 *chip = dev_get_priv(dev);
	u32 interrupt, phandle;
	int ret;
	printf(">>> ENTER %s.\n", __func__);
	phandle = dev_read_u32_default(dev, "interrupt-parent", -ENODATA);
	if (phandle == -ENODATA) {
		printf("bq25700: Read 'interrupt-parent' failed, ret=%d\n", phandle);
		return phandle;
	}

	ret = dev_read_u32_array(dev, "interrupts", &interrupt, 1);
	if (ret) {
		printf("bq25700: Read 'interrupts' failed, ret=%d\n", ret);
		return ret;
	}

	chip->irq = phandle_gpio_to_irq(phandle, interrupt);
	if (chip->irq < 0)
		printf("bq25700: Failed to request bq25700 irq, ret=%d\n", chip->irq);

	irq_install_handler(chip->irq, bq25700_irq_handler, dev);
