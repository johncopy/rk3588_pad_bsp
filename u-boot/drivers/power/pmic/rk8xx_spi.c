...
#ifdef CONFIG_DELAY_POWERON
/*
return 0:not pressed 1: pressed
*/
struct udevice *master_dev=NULL;
#define RK806_SYS_STS  0x5D
int rk8xx_spi_read_pwrkey_status(void)
{
	u8 value=0;
	if(master_dev){
		rk806_spi_read(master_dev,RK806_SYS_STS, &value, 1);
		printf("%s>>>>get rk806 pwrkey RK806_SYS_STS=0x%x\n",__func__,value);
	}
	return !(value&0x80);
}
#endif
...
static int rk8xx_spi_probe(struct udevice *dev)
{
	...
	#ifdef CONFIG_DELAY_POWERON
		if(!strcmp(dev->name, "rk806master@0")){
			master_dev=dev;
			rk806_spi_read(dev,RK806_PWRON_KEY, &value, 1);
			value|=0x30;
			rk806_spi_write(dev,RK806_PWRON_KEY, &value, 1);
			printf("%s>>>>dual pmic,get master udevice\n",__func__);
		}
	#endif
	} else {
	...
	#ifdef CONFIG_DELAY_POWERON
		if(!strcmp(dev->name, "rk806single@0")){
			master_dev=dev;
			rk806_spi_read(dev,RK806_PWRON_KEY, &value, 1);
			value|=0x30;
			rk806_spi_write(dev,RK806_PWRON_KEY, &value, 1);
			printf("%s>>>>single pmic,get master udevice\n",__func__);
		}
	#endif
	}
