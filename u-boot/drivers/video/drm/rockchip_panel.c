...
static void panel_simple_prepare(struct rockchip_panel *panel)
{
	...
	if (dm_gpio_is_valid(&priv->vsp_gpio))
		dm_gpio_set_value(&priv->vsp_gpio, 1);

	if (plat->delay.vsp)
		mdelay(plat->delay.vsp);

	if (dm_gpio_is_valid(&priv->vsn_gpio))
		dm_gpio_set_value(&priv->vsn_gpio, 1);

	if (plat->delay.vsn)
		mdelay(plat->delay.vsn);
...
static void panel_simple_unprepare(struct rockchip_panel *panel)
{
	...
	if (dm_gpio_is_valid(&priv->vsn_gpio))
		dm_gpio_set_value(&priv->vsn_gpio, 0);

	if (plat->delay.vsn)
		mdelay(plat->delay.vsn);

	if (dm_gpio_is_valid(&priv->vsp_gpio))
		dm_gpio_set_value(&priv->vsp_gpio, 0);
...
static int rockchip_panel_ofdata_to_platdata(struct udevice *dev)
{
	...
	plat->delay.vsp = dev_read_u32_default(dev, "vsp-delay-ms", 0);
	plat->delay.vsn = dev_read_u32_default(dev, "vsn-delay-ms", 0);
...
static int rockchip_panel_probe(struct udevice *dev)
{
	...
	ret = gpio_request_by_name(dev, "vsp-gpios", 0,
				   &priv->vsp_gpio, GPIOD_IS_OUT);
	if (ret && ret != -ENOENT) {
		printf("%s: Cannot get vsp GPIO: %d\n", __func__, ret);
		return ret;
	}

	ret = gpio_request_by_name(dev, "vsn-gpios", 0,
				   &priv->vsn_gpio, GPIOD_IS_OUT);
	if (ret && ret != -ENOENT) {
		printf("%s: Cannot get vsn GPIO: %d\n", __func__, ret);
		return ret;
	}
