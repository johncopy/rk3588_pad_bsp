...
extern int rk8xx_spi_read_pwrkey_status(void);
static void uboot_check_pwrkey(void)
{
	const char *preboot = env_get("preboot");
	struct udevice *pmic;
	int boot_mode,ret;
	extern int auto_exit_uboot;
	ulong delta;
	/* If there is preboot command, exit */
	if (preboot && !strstr(preboot, "dvfs")) {
		printf("Exit %s: due to preboot cmd '%s'\n",__func__,preboot);
		return;
	}
	...
	{
		printf("Exit %s: due to boot mode\n", __func__);
		return;
	}
#endif
	...
	if (ret) {
		if (ret == -ENODEV)
			printf("%s>>>Can't find PMIC\n",__func__);
		else
			printf("%s>>>>Get UCLASS PMIC failed: %d\n", __func__,ret);
		return;
	}
	...
	while(1){
		if(rk8xx_spi_read_pwrkey_status() == 0) {
			printf("%s>>>>pwrkey is not pressed,so shutdown system.\n",__func__);
			pmic_shutdown(pmic);
			mdelay(1000);
		} else {
			mdelay(100);
			if(get_timer(delta) > 1000) {
				printf("%s>>>>pwrkey is pressed more than 3s,open system.\n",__func__);
				break;
			}
		}
	};
